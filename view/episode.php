<div class="mx-auto col-lg-9">
		...
		<nav aria-label="pagination" class="mt-3">
			<ul class="pagination d-flex justify-content-center">
				...
				if ($episode->getChapter() > 1) {
					?>
					<li class="page-item order-0"><a class="page-link" href="episode=<?= $episode->getChapter() - 1; ?>">Précédent</a></li>
					<?php
				}
				$lastChapter = end($episodes)->getChapter(); 
				if ($episode->getChapter() < $lastChapter) {
					?>
					<li class="page-item order-2"><a class="page-link" href="episode=<?= $episode->getChapter() + 1; ?>">Suivant</a></li>
					<?php
				}
				?>
				...
				foreach($episodes as $episodePage) {
					?>
					<li class="page-item order-1"><a class="page-link" href="episode=<?= $episodePage->getChapter(); ?>"><?= $episodePage->getChapter(); ?></a></li>
					<?php
				}
				?>
			</ul>
	...
	if (isset($comments) && isset($_SESSION['login'])) {
		...
		<div class="mt-3">
			...
			<div>
				<form method="post" action="addComment=<?= strip_tags($episode->getChapter()); ?>">
					...
					<div class="form-group">
						...
						<a href="episode=<?= strip_tags($episode->getChapter()); ?>"><input type="button" value="Annuler" class="btn btn-dark" /></a>
					</div>
				</form>
			</div>
				...
				foreach ($comments as $comment) {
					...
					<tr id="commentaires">
						<td>
							...
							<p><a href="rudeComment-<?=strip_tags($comment->getId()); ?>#commentaires"><input type="button" value="Signaler le commentaire" class="btn btn-outline-secondary" /></a></p>
						</td>
					</tr>
					<?php
				}
